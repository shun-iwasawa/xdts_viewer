...
enum FrameOptionId {
  FrameOption_None = 0,
  FrameOption_KeyFrame,
  FrameOption_ReferenceFrame
};

struct FrameData {
  QString frame;
  FrameOptionId option;
};
...
public:
  ...
  static QString SYMBOL_TICK_1, SYMBOL_TICK_2, SYMBOL_NULL_CELL, SYMBOL_HYPHEN;

  XdtsFrameDataItem() : m_id(Default) {}
  XdtsFrameDataItem(QString fId) : m_id(Default) { m_values.append(fId); }
  void read(const QJsonObject& json);
  void write(QJsonObject& json) const;
...
public:
  ...
  XdtsTrackFrameItem(int frame, QString fId) : m_frame(frame) {
    m_data.append(XdtsFrameDataItem(fId));
  }
  void read(const QJsonObject& json);
  void write(QJsonObject& json) const;
  QPair<int, FrameData> frameFdata() const;
};
...
public:
  ...
  QVector<FrameData> getCellFrameDataTrack(QList<int>& tick1, QList<int>& tick2,
                                           bool& isHoldFrame) const;

  void addFrame(int frame, QString fId) {
...
public:
  ...
  QVector<FrameData> getColumnTrack(int col, QList<int>& tick1,
                                    QList<int>& tick2, bool& isHoldFrame) const;
};
